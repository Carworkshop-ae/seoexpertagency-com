...
This guide takes a fresh machine to a running production site at `https://seoexpertsagency.com`.
...
- **Domain:** `seoexpertsagency.com` with DNS you control
...
VALUES ('<user-uuid-from-auth-users>', 'admin@seoexpertsagency.com', 'Admin', 'super_admin');
```
...
- Domains → Add `seoexpertsagency.com` → add the SPF/DKIM/DMARC DNS records → verify (≤24 h)
- From address used by code: `noreply@seoexpertsagency.com` (overridable in Admin → Settings → Email)
...
- analytics.google.com → property for `seoexpertsagency.com` → Measurement ID `G-XXXXXXXXXX`
...
| `NEXT_PUBLIC_SITE_URL` | ✅ | `https://seoexpertsagency.com` (per-env: use the preview URL on Preview) |
...
Add `seoexpertsagency.com` and `www.seoexpertsagency.com`. At your registrar:
```
A      @     76.76.19.19
...
```bash
curl -I https://seoexpertsagency.com    # HTTP/2 200, server: Vercel
```
...
```bash
curl -I https://seoexpertsagency.com                       # 200
curl -I https://seoexpertsagency.com/admin                 # 302 → /admin/login
curl -s -o /dev/null -w "%{http_code}\n" \
     https://seoexpertsagency.com/api/admin/services       # 401
curl -s https://seoexpertsagency.com/robots.txt            # Disallow: /admin/  + sitemap-index
curl -s https://seoexpertsagency.com/sitemap.xml           # valid XML <sitemapindex>
curl -I https://seoexpertsagency.com/services/local-seo    # x-nextjs-cache: MISS, then HIT on 2nd
```
- Log in at `https://seoexpertsagency.com/admin/login` with the Step 1.6 credentials.
...
search.google.com/search-console → add property `https://seoexpertsagency.com` → verify via DNS TXT
→ Sitemaps → submit `https://seoexpertsagency.com/sitemap.xml`.
...
Common settings — URL `https://seoexpertsagency.com/api/revalidate`, method POST, headers
...
```bash
curl -X POST https://seoexpertsagency.com/api/revalidate \
...
```bash
npx lighthouse https://seoexpertsagency.com --only-categories=performance,seo,accessibility --view
```
