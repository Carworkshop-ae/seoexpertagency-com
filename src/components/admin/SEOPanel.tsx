...
interface SEOPanelProps {
  ...
  pageUrl?: string            // e.g. https://seoexpertsagency.com/services/technical-seo
  defaultTitle?: string
...
export function SEOPanel({ value, onChange, pageUrl, defaultTitle = '', defaultDescription = '', autoSchemas = [] }: SEOPanelProps) {
  ...
  const crumb = (pageUrl ?? 'https://seoexpertsagency.com').replace(/^https?:\/\//, '').replace(/\//g, ' › ')
    ...
    <div className="space-y-4">
      ...
      <AdminSectionCard title="Basic SEO">
        <div>
          ...
          <input value={title} onChange={e => set('meta_title', e.target.value)} className={inputCls} placeholder={defaultTitle || 'Technical SEO Services in Dubai | SEO Expert Agency'} />
        </div>
        <div>
          ...
          <textarea value={desc} onChange={e => set('meta_description', e.target.value)} rows={3} className={inputCls} placeholder={defaultDescription || 'Technical SEO audits that fix crawl, indexing and Core Web Vitals issues…'} />
        </div>
      ...
      <AdminSectionCard title="Open Graph / Social" defaultOpen={false}>
        ...
        <div className="rounded-lg border border-zinc-200 overflow-hidden bg-white">
          ...
          <div className="p-3">
            <p className="text-[11px] uppercase text-zinc-400">seoexpertsagency.com</p>
